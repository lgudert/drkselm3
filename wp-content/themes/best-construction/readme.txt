=== Best Construction ===
Contributors: CooThemes
Tags: Blog, Custom Background, Custom Colors, Custom Header, Custom Logo, Custom Menu, E-Commerce, Featured Images, Flexible Header, Footer Widgets, Full Width Template, Left Sidebar, Portfolio, Right Sidebar, Theme Options, Translation Ready, Two Columns

Requires at least: 4.0
Tested up to: 6.11
Stable tag: 1.1.2

Author URI:  https://www.coothemes.com/
Theme URI:  https://www.coothemes.com/theme-best-construction/
License: GNU General Public License v2 or later
License URI: License URI: http://www.gnu.org/licenses/gpl-3.0.html


=== Description ===
Best Construction is a fully responsive WordPress theme built for businesses like architecture, building, company, construction, constructor, contractor, corporate, industry, metal, painter, plumber, plumbing, remodeling, renovation, roofing and other construction related businesses. The theme is purpose-oriented designed and comes with loads of awesome features, such as gallery, portfolio, HTML5 video background, call to action, Google Map and subscribe form, to perfectly present your company in a very pleasant and professional way. Moreover, you can easily re-create the front-page or part of it on any other page of your website. The theme is fully compatible with WooCommerce, Contact Form 7 and MailChimp. And it's highly customized and allows you to build a unique website without editing a single line of code. Demo: http://demo.coothemes.com/best-construction-pro/

=== License ===
Best Construction WordPress Theme, Copyright 2018 Coothemes.com
Best Construction is distributed under the terms of the GNU General Public License v2 

Best Construction WordPress Theme is the child theme of Theta theme, Copyright 2018 Coothemes.com
Best Construction is distributed under the terms of the GNU General Public License v2 

=== Resources ===
     animate.css, Copyright 2016 Daniel Eden
     License: MIT license
     Source:  http://opensource.org/licenses/MIT


     jquery.waypoints.js, Copyright 2011-2015 Caleb Troughton
     License: MIT license
     Source:  https://github.com/imakewebthings/waypoints/blob/master/licenses.txt


     parallax.js, copyright 2016 PixelCog, Inc.
     License: MIT license
     Source:  https://github.com/pixelcog/parallax.js/blob/master/LICENSE


     customizer pro, copyright 2016,  Justin Tadlock
     License: GNU GPL, version 2 or later
     Source:  https://github.com/justintadlock/trt-customizer-pro/blob/master/license.md  

     Coothemes icon, copyright 2018,  Coothemes.com
     License: GNU GPL, version 2 or later
     Source:  https://www.coothemes.com/icon/


     tgm plugin, copyright 2011,  Thomas Griffin
     License: GPL v2 or later
     Source:  http://opensource.org/licenses/gpl-2.0.php  


    default.jpg, Copyright Pxhere.com
    License: CC0 1.0 Universal (CC0 1.0)
    Source: https://pxhere.com/en/photo/1087241 


    screenshot.jpg, Copyright Pxhere.com
    License: CC0 1.0 Universal (CC0 1.0)
    Source: https://pxhere.com/en/photo/513138


=== Frequently Asked Questions ===

Does this theme have a support forum? 

Theta provides support at https://www.coothemes.com/forum/theta-all-theme/best-construction-theme/


=== Changelog ===
version 1.1.2 20230119
* Compatible with wordpress to version 6.1.1

version 1.1.1 20220224
* Compatible with wordpress to version 5.9.1.

=version 1.1 20210115=
*update images

=version 1.0.9 20210103=
*fix news problem
*Compatible with wordpress to version 5.6.0

=version 1.0.8 20181103=
*fix news problem

=version 1.0.7 20181018=
*adjust slider button & title height

=version 1.0.6 20181015=
*mod  copyright

=version 1.0.5 20181004=
*fix Recommended actions link

=version 1.0.4 20180923=
*fix blog section

=version 1.0.3 20180803=
*modify link

=version 1.0.1 20180710=
*update default.jpg

=version 1.0.0 20180622=

- initial release
--------------------------------------------------------------------------------------------

	For any help you can contact us at support[at]coothemes.com
