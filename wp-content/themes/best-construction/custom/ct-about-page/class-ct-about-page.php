...
if ( ! class_exists( 'Coothemes_About_Page' ) ) {
	...
	class Coothemes_About_Page {
		...
		public function setup_actions() {
			...
			add_action( 'wp_ajax_ct_about_page_dismiss_required_action', array( $this, 'dismiss_required_action_callback' ) );
		...
		public function Coothemes_about_page_render() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<a href="'.esc_url('https://www.coothemes.com/').'" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
		...
		public function call_plugin_api( $slug ) {
			...
			$call_api = get_transient( 'ct_about_plugin_info_' . $slug );
			...
			if ( false === $call_api ) {
				...
				set_transient( 'ct_about_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
			}
		...
		public function get_plugin_icon( $arr ) {
			...
			if ( ! empty( $arr['svg'] ) ) {
				...
				$plugin_icon_url = get_template_directory_uri() . '/ct-about-page/images/placeholder_plugin.png';
			}
		...
		public function recommended_actions() {
			...
			if ( ! empty( $recommended_actions ) ) {
				...
				if ( ! empty( $actions ) && is_array( $actions ) ) {
					...
					$ct_about_page_show_required_actions = get_option( $this->theme_slug . '_required_actions' );
					...
					foreach ( $actions as $action_key => $action_value ) {
						...
						if ( $ct_about_page_show_required_actions[ $action_value['id'] ] === false ) {
							$hidden = true;
						}
						...
						echo '<div class="ct-about-page-action-required-box">';

						if ( ! $hidden ) {
							echo '<span data-action="dismiss" class="dashicons dashicons-visibility ct-about-page-required-action-button" id="' . esc_attr( $action_value['id'] ) . '"></span>';
						} else {
							echo '<span data-action="add" class="dashicons dashicons-hidden ct-about-page-required-action-button" id="' . esc_attr( $action_value['id'] ) . '"></span>';
						}
		...
		public function child_themes() {
			echo '<div id="child-themes" class="ct-about-page-tab-pane">';
				...
				if ( ! empty( $child_themes['content'] ) && is_array( $child_themes['content'] ) ) {
					...
					for ( $i = 0; $i < count( $child_themes['content'] ); $i ++ ) {
						...
						if ( ! empty( $child['image'] ) ) {
							echo '<div class="ct-about-child-theme">';
							echo '<div class="ct-about-page-child-theme-image">';
							echo '<img src="' . esc_url( $child['image'] ) . '" alt="' . ( ! empty( $child['image_alt'] ) ? esc_html( $child['image_alt'] ) : '' ) . '" />';
							if ( ! empty( $child['title'] ) ) {
								echo '<div class="ct-about-page-child-theme-details">';
							...
							echo '</div><!--ct-about-page-child-theme-image-->';
							echo '</div><!--ct-about-child-theme-->';
						}// End if().
					}// End for().
					echo '</div>';
				}// End if().
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo '<div id="free_pro" class="ct-about-page-tab-pane ct-about-page-fre-pro">';
					...
					if ( ! empty( $free_pro['pro_theme_link'] ) && ! empty( $free_pro['get_pro_theme_label'] ) ) {
						echo '<tr class="ct-about-page-text-center">';
						echo '<td></td>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'ct-about-page-css', THEME_URL . '/custom/ct-about-page/css/ct_about_page_css.css', array(), THETA_VERSION );

			if ( 'appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix ) {

				wp_enqueue_script( 'ct-about-page-js', THEME_URL . '/custom/ct-about-page/js/ct_about_page_scripts.js', array( 'jquery' ), THETA_VERSION );
				...
				wp_localize_script(
					'ct-about-page-js', 'ctAboutPageObject', array(
						...
						'activating_string'   => esc_html__( 'Activating', 'best-construction' ),
					)
				);
			...
			if ( ! empty( $action_id ) ) {
				...
				if ( get_option( $this->theme_slug . '_required_actions' ) ) {
					...
					switch ( esc_html( $_GET['todo'] ) ) {
						case 'add':
							$ct_about_page_show_required_actions[ absint( $action_id ) ] = true;
							break;
						case 'dismiss':
							$ct_about_page_show_required_actions[ absint( $action_id ) ] = false;
							break;
					}

					update_option( $this->theme_slug . '_required_actions', $ct_about_page_show_required_actions );

					/* create the new option,with false for the specified id */
				} else {

					$ct_about_page_show_required_actions_new = array();

					if ( ! empty( $recommended_actions ) ) {

						foreach ( $recommended_actions as $ct_about_page_required_action ) {

							if ( $ct_about_page_required_action['id'] == $action_id ) {
								$ct_about_page_show_required_actions_new[ $ct_about_page_required_action['id'] ] = false;
							} else {
								$ct_about_page_show_required_actions_new[ $ct_about_page_required_action['id'] ] = true;
							}
						}

						update_option( $this->theme_slug . '_required_actions', $ct_about_page_show_required_actions_new );
